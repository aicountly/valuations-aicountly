...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://valuations.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://valuations.gh.aicountly.com (+ `/api`) |
...
`https://valuations.aicountly.com/api/.env` unless Apache is told otherwise.
