# AICOUNTLY auth workflow (Valuations)

How Valuations signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `valuations.aicountly.com` (or `valuations.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/valuations?returnUrl={origin}/auth/callback`.
...
| Valuations host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `valuations.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `valuations.aicountly.com/api` |
| `valuations.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `valuations.gh.aicountly.com/api` |
...
`valuations` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://valuations.gh.aicountly.com/api/health
...
curl -i -X POST https://valuations.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://valuations.gh.aicountly.com/api/global/login   # expect 404
```
